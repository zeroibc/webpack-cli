...
const noJquery = require('publicDir/libs/without-jquery.module');
const layout = require('./html.ejs'); // 整个页面布局的模板文件，主要是用来统筹各个公共组件的结构
const header = require('cmpDir/header/html.ejs'); // 页头的模板
const footer = require('cmpDir/footer/html.ejs'); // 页脚的模板
const topNav = require('cmpDir/top-nav/html.ejs'); // 顶部栏的模板
const slideShow = require('cmpDir/slide-show/html.ejs'); // 轮播的模板
...
const moduleExports = {
  ...
  run(content) {
    ...
    const renderData = {
      ...
      slideShow: slideShow(componentRenderData),
      content,
    };
